...
public final class ContestCounter {
   ...
   **/
  public static ContestResult countContest(final Contest contest) {
    final Set<CountyContestResult> countyResults =
      CountyContestResultQueries.forContest(contest);
    final ContestResult contestResult = ContestResultQueries.forContest(contest);
    contestResult.setVoteTotals( accumulateVoteTotals(countyResults.stream()
                                                      .map((cr) -> cr.voteTotals())
                                                      .collect(Collectors.toList())));
    // contestResult.getMargins
